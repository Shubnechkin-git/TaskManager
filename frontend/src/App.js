...
const App = () => {
  ...
  const go = (e, k = 0) => {
    if (k == 0) setActivePanel(e.currentTarget.dataset.to);
    else setActivePanel(e);
  };
  ...
  useEffect(() => {
    ...
    const fetchUserId = async () => {
      try {
        ...
        axios
          .post("https://taskmanagerbackend.cleverapps.io/user/create_user", {
            userInfo,
          })
          .then((response) => {
  ...
  useEffect(() => {
    const onlineHandler = () => {
      const parsedData = JSON.parse(localStorage.getItem("sliderIsViewed"));
      if (parsedData === true) setActivePanel("home");
      // Установка активной панели по умолчанию при восстановлении соединения
      else setActivePanel("slider"); // Установка активной панели по умолчанию при восстановлении соединения
    };
  ...
  const handleUpdate = () => {
    ...
    axios
      .put("https://taskmanagerbackend.cleverapps.io/counters/update_counter", {
        id: idCounter,
        vk_id: userId,
        title: titleEdit,
      })
  ...
  const getTasksUser = () => {
    if (userId > 0) {
      axios
        .get("https://taskmanagerbackend.cleverapps.io/tasks/get", {
          params: {
            id: userId,
          },
        })
  ...
  const handleCreate = (k = 0) => {
    if (title !== null) {
      axios
        .post(
          "https://taskmanagerbackend.cleverapps.io/counters/create_counter",
          { title, vk_id: userId }
        )
        .then((response) => {
          console.log(response);
          if (response.data.success) {
            ...
            openSuccess(response.data.message);
          } else openError(response.data.message);
        })
        .catch((error) => {
          console.log(error);
          openError(error.response.data.message);
  ...
  const fetchCounters = (k = 0) => {
    if (k == 1) setUserCounters([]);

    if (userId > 0) {
      axios
        .get("https://taskmanagerbackend.cleverapps.io/counters/get", {
          params: {
            id: userId,
          },
        })
    ...
    <ConfigProvider appearance="dark">
      <AdaptivityProvider>
            ...
            <SplitCol>
              {activePanel !== "slider" && (
                <HeaderMy
                  ...
                  displayName={activePanel}
                  go={go}
                  leftBtn={<CreateQuestBtn go={go} />}
                />
              )}
              {activePanel !== null ? (
        ...
        <ModalRoot activeModal={activeModal}>
          ...
          <ModalCard
            header="Редактирование счетчика"
            onClose={(e) => handleCLose()}
            id="edit"
          >
            <FormItem htmlFor="example" top="📝 Название">
